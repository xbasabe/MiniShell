...
void	redir_stack_fb(t_stack *stack)
{
	...
	//if (tmp_back->pipe.redir != 'b'
	//	|| tmp_back->pipe.redir != 'h' || tmp_back->pipe.redir != 'p')
	//	tmp_back->pipe.p[1] = 1;
	while (tmp_back != NULL)
	{
		//printf("B cmd %s [%d] [%d]", tmp->pipe.cmd, tmp->pipe.p[0], tmp->pipe.p[1]);
...
t_stack	*back_redir_count(t_stack *tmp)
{
	...
	tp = tmp;
	while (tp->next != NULL
		&& (tp->next->pipe.redir == 'b' || tp->next->pipe.redir == 'h' ))
		tp = tp->next;
	return (tp);
}
