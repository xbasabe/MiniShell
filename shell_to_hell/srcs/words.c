...
char	**get_words(char *txt, int *count, int *flags) //count numero de comillas
{
	...
	j = 0;
	c_words = 0;
	printf("Linea a cortar en plabras: %s\n", txt);
	...
	while(txt && txt[i] != '\0')
	{
		...
		else if(txt[i] == ' ') // tabulador? crear funcion is_space?
        {
		    temp_text[j] = '\0';
			words[c_words] = (char*)malloc(sizeof(char) * 999);
    ...
    strcpy(words[c_words], temp_text); //strdup?
	printf("     Word[%d] %s\n", c_words, words[c_words]);
	temp_text[0] = '\0';
	c_words++;
