...
char *expand_vble_word(char *txt, int init)
{
	...
	while (txt[i] != '$')
		i++;
	expanded = (char*)malloc(sizeof(char) * i);
	ft_strlcpy(expanded, txt, i);
	tmp = search_vble_env(txt, init);
	if(tmp == NULL)
		return("\n");
	value = (char*)malloc(sizeof(char) * (ft_strlen(tmp) + 1));
	value = strcpy(value, tmp);
	ft_strjoin(expanded, value);
