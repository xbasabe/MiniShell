...
int	echo(t_stack *node, char *input)
{
	...
	(void)input;
	output = NULL; // para parsear, output para comprobar nuevo parseo
	i = -1;
	n = 0;
	...
	if (n == 1)
		fd_putstr_out("\n", node);
	/*
	printf("Prueba parseo: \n");
	//reiniciamos contadores
	i = -1;
	n = 0;
	output = split_tokens(input);
	if (strcmp(output[0], "-n"))
	{
		n = 1;
		i++;
	}
